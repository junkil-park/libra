...
impl<'env> ModuleEnv<'env> {
    ...
    /// Gets a FunctionEnv by FunctionHandleIndex.
    /// The returned function might be from this or another module.
    pub fn get_function_by_handle_idx(&self, idx: FunctionHandleIndex) -> FunctionEnv<'_> {
        let view =
    ...
    /// Gets a FunctionEnv by FunctionInstantiationIndex.
    /// The returned function might be from this or another module.
    pub fn get_function_by_inst_idx(&self, idx: FunctionInstantiationIndex) -> FunctionEnv<'_> {
        self.get_function_by_handle_idx(self.data.module.function_instantiation_at(idx).handle)
    }
        ...
        self.get_bytecode()
            ...
            .filter_map(|c| {
                if let Bytecode::Call(i) = c {
                    Some(
                        self.module_env
                            .get_function_by_handle_idx(*i)
                            .get_qualified_id(),
                    )
                } else if let Bytecode::CallGeneric(i) = c {
                    Some(
                        self.module_env
                            .get_function_by_inst_idx(*i)
                            .get_qualified_id(),
                    )
                } else {
                    None
